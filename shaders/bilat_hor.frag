...
/*
    for(int i = -FILTER_RADIUS;i <= FILTER_RADIUS; i++) {
        vec3 reading = max(vec3(0),textureOffset(prev_bounce,vec3(fragUV,float(layer)),ivec2(i,0)).rgb);

        float Z_reading = textureOffset(depth_texture,vec3(fragUV,float(layer)),ivec2(i,0)).r;

        float edge = textureOffset(edge_texture,vec3(fragUV,float(layer)),ivec2(i,0)).x;
...
*/
    ...
    for(int i = -1; i <= 1; i += 2) {
        for(int j = 1; j <= FILTER_RADIUS; j++) {
            float edge = textureOffset(edge_texture,vec3(fragUV,float(layer)),ivec2(i*j,0)).r;

            vec3 reading = max(vec3(0),textureOffset(prev_bounce,vec3(fragUV,float(layer)),ivec2(i*j,0)).rgb);
